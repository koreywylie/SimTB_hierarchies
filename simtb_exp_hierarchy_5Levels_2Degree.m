...
HIER_levels = 5;  % number of levels in hierarchy
HIER_degree = 2;  % size of parcellation at each level, defaults to 2 in get_fractal_SM()
HIER_equal_size = true; % if false, partitions drawn from Dirichlet r.v. w/ param a = (HIER_degree,..,HIER_degree)
HIER_groupSM_file = fullfile(out_path, [prefix, '_groupSM.mat']);  % path to file w/ shared group spatial map
...
% TC_unique_prob = unifrnd(0.2, 0.6, 1, nC); % [1 x nC] prob of unique event at each TR, w/ event heterogeneity
% 7/28/2022 --kw-- NOTE: TC_unique_prob must be [1 x nC], specification below & in parts of manual incorrect
% TC_unique_prob = repmat(unifrnd(0.3 ,0.6, 1, nC),M,1); % [M x nC] prob of unique event at each TR, w/ event heterogeneity
